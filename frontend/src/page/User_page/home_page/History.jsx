...
import CartHeader from "../cart_pages/CartHeader";
...
export default function History() {
  ...
  const formatDate = (dateString) => {
    const reqTime = new Intl.DateTimeFormat("en-GB", {
      day: "2-digit",
      month: "short",
      year: "numeric",
      hour: "2-digit",
  ...
  // Fetch order items
  useEffect(() => {
    const user_id = getUserIdData(); // Retrieve the user_id from local storage

    if (user_id) {
      axios
        .get(`${import.meta.env.VITE_SERVER_URL}/orders/user/${user_id}`)
        .then((response) => {
          if (response.data) {
            const sortedOrders = response.data.sort(
              (a, b) => new Date(b.timing) - new Date(a.timing)
            );
            setOrderitems(sortedOrders); // Set fetched order data to state
            setCurrentIndexes(sortedOrders.map(() => 0)); // Initialize indexes for display
      ...
      const intervalId = setInterval(() => {
        setCurrentIndexes((prevIndexes) =>
          prevIndexes.map((currentIndex, i) => {
            const totalOrders = orderitems[i]?.orders?.length || 0;
  ...
  }, [orderitems, pauseAutoScroll]);

  // Handle manual scrolling
  const handleManualScroll = (direction, productIndex) => {
    setPauseAutoScroll(true);
    setCurrentIndexes((prevIndexes) => {
      const newIndexes = [...prevIndexes];
      const totalOrders = orderitems[productIndex]?.orders?.length || 0;
      if (totalOrders === 0) return prevIndexes;

      if (direction === "next") {
        newIndexes[productIndex] =
          (prevIndexes[productIndex] + 1) % totalOrders;
      } else if (direction === "prev") {
        newIndexes[productIndex] =
          (prevIndexes[productIndex] - 1 + totalOrders) % totalOrders;
      }
  ...
  return (
    <div className="bg-customLightGray4 min-h-screen max-h-fit">
      <div ref={historyRef} className="sticky top-0 z-50">
        <CartHeader title="Order History" homeRoute="/home" />
      </div>
      {!historyLoading ? (
        <div className="mt-3">
          {orderitems.length > 0 ? (
            orderitems.map((item, index) => (
              ...
              >
                <div className="w-[95%] rounded-md border lg:h-auto bg-white shadow-lg mb-4 sm:w-[50%] relative">
                  <div className="px-2 pt-1">
                    <div className="flex justify-between bg-white rounded-md py-1 relative z-40">
                      {item.orders.length > 1 && (
                        <AiOutlineLeft
                          onClick={() => handleManualScroll("prev", index)} // Pass product index
                          className="absolute left-1 top-[82%] transform -translate-y-1/2 bg-white rounded-full p-2 cursor-pointer hover:bg-gray-300 z-30"
                          size={30}
                        />
                      )}
                      <div className="relative w-full overflow-hidden rounded-md">
                        ...
                        >
                          {item.orders.map((order) => (
                            <div
                              key={order.id}
                              className={`flex flex-shrink-0 min-w-full w-full pt-4 ${
                                item.orders.length > 1 ? "pb-9" : "pb-4"
                              } bg-gray-200`}
                            >
                                ...
                                {order.veg_non_veg === 2 && (
                                  <div>
                                    ...
                                    >
                                      ...
                                      />
                                      <path
                                        d="M7.39317 54.297C5.7691 53.1804 5.76909 50.7444 8.00219 49.4248H73.7766C74.8139 50.0518 75.2036 50.5955 75.6037 51.8609C75.3141 53.2969 74.8992 53.8851 73.7766 54.6015L7.39317 54.297Z"
                                        fill="#0B1D78"
                                        stroke="black"
                                        strokeWidth="2"
                                      />
                                      <path
7011 69.8691 46.9032 69.415L49.0346 64.6602H49.1181L51.267 69.5469C51.4721 70.0098 51.6406 70.3013 51.7724 70.4214C51.872 70.5122 52.0126 70.5649 52.1943 70.5796V70.7422H49.3115V70.5796H49.4301C49.6616 70.5796 49.8241 70.5474 49.9179 70.4829C49.9824 70.436 50.0146 70.3687 50.0146 70.2808C50.0146 70.228 50.0058 70.1738 49.9882 70.1182C49.9824 70.0918 49.9384 69.9819 49.8564 69.7886L49.54 69.0459ZM49.3906 68.7207L48.5029 66.6685L47.5888 68.7207H49.3906ZM54.0459 64.7832L56.9859 68.479V65.917C56.9859 65.5596 56.9346 65.3179 56.8321 65.1919C56.6915 65.022 56.4556 64.9399 56.1246 64.9458V64.7832H58.0933V64.9458C57.8414 64.978 57.6714 65.0205 57.5835 65.0732C57.4986 65.123 57.4312 65.2065 57.3814 65.3237C57.3345 65.438 57.3111 65.6357 57.3111 65.917V70.8784H57.1617L53.1319 65.917V69.7051C53.1319 70.0479 53.2095 70.2793 53.3648 70.3994C53.523 70.5195 53.7032 70.5796 53.9053 70.5796H54.0459V70.7422H51.9322V70.5796C52.2603 70.5767 52.4888 70.5093 52.6177 70.3774C52.7466 70.2456 52.8111 70.0215 52.8111 69.7051V65.4995L52.6836 65.3413C52.5577 65.1831 52.4463 65.0791 52.3497 65.0293C52.253 64.9795 52.1138 64.9517 51.9322 64.9458V64.7832H54.0459Z"
                                        fill="black"
                                      />
                                    </svg>
                                  </div>
                      ...
                      {item.orders.length > 1 && (
                        <>
                          <div className="absolute bottom-0 left-0 right-0 flex justify-center items-center gap-2 py-2">
                            {item.orders.map((_, dotIndex) => (
                              <div
                                key={dotIndex}
                                onClick={() => {
                                  setPauseAutoScroll(true);
                          ...
                          <AiOutlineRight
                            onClick={() => handleManualScroll("next", index)} // Pass product index
                            className="absolute right-1 top-[82%] transform -translate-y-1/2 bg-white rounded-full p-2 cursor-pointer hover:bg-gray-300 z-40"
                            size={30}
                          />
                        </>
                      )}
                  ...
                  <div className="flex-col pb-3 flex justify-start rounded-b-md px-3 pt-2">
                    <div className="text-xl text-black font-bold mb-2">
                      Order ID:{" "}
                      <span className="text-gray-800">{item.orderid}</span>
                    </div>
                    <div className="flex items-center mb-2">
                      <TiShoppingCart className="text-xl mr-1 text-black" />
                    ...
                    <div className="flex items-center mb-2 text-black">
                      <MdOutlineAccessTimeFilled className="text-xl mr-1" />
                      <span className="text-md">{formatDate(item.timing)}</span>
                    </div>
                    ...
                    <p className="mt-2">
                      Payment:{" "}
                      <span
                        className={`px-1 py-1 rounded-full font-semibold ${
                          item.paymentStatus
                            ? " text-green-600"
                            : " text-pureRed"
                        }`}
            ...
            <div className="flex flex-col gap-4 justify-center items-center h-screen py-4">
            ...
            <div className="flex justify-center items-center h-screen py-4">
              <h1 className="text-2xl md:text-3xl text-center">
                No History! Just order something
              </h1>
            </div>
          )}
        </div>
        ...
        <div className="flex flex-col gap-3 justify-center items-center h-screen">
          ...
          <div className="text-xl :text-3xl">Loading your history</div>
        </div>
      )}
    </div>
  );
